...
When a request fails, the Revenexx Go SDK returns a `*client.RevenexxError` as the `error` value. Type-assert the error to inspect it, or simply present its message.
...
if _, err := service.ProductsGet("<PRODUCT_ID>"); err != nil {
    if apiErr, ok := err.(*client.RevenexxError); ok {
