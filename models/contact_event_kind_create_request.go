...
// ContactEventKindCreateRequest Add one value to the activity types set. It
// is available to `contact_events.kind` immediately.
type ContactEventKindCreateRequest struct {
    // What `contact_events.kind` will store. Lowercase, starting with a letter;
...
func (model ContactEventKindCreateRequest) New(data []byte) *ContactEventKindCreateRequest {
    model.data = data
    return &model
}

func (model *ContactEventKindCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
