...
// ContactCreateRequest Creates the contact (system of record) and mirrors it
// as a platform user (status defaults to invited).
...
func (model ContactCreateRequest) New(data []byte) *ContactCreateRequest {
    model.data = data
    return &model
}

func (model *ContactCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
