...
// FormUpdateRequest Partial update — omitted fields keep their current
// value.
...
func (model FormUpdateRequest) New(data []byte) *FormUpdateRequest {
    model.data = data
    return &model
}

func (model *FormUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
