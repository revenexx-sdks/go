...
// ShippingCarrierUpdateRequest Partial update — omitted fields keep their
// current value.
type ShippingCarrierUpdateRequest struct {
...
func (model ShippingCarrierUpdateRequest) New(data []byte) *ShippingCarrierUpdateRequest {
    model.data = data
    return &model
}

func (model *ShippingCarrierUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
