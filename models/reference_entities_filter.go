...
// ReferenceEntitiesFilter The exact-column filters this call was understood
// to carry, verbatim as they arrived. A query parameter that is not a column
// of `reference_entities` — `?status=`, a typo, a filter another entity has
...
// matched".
type ReferenceEntitiesFilter struct {
...
func (model ReferenceEntitiesFilter) New(data []byte) *ReferenceEntitiesFilter {
    model.data = data
    return &model
}

// Use this method to get response in desired type
func (model *ReferenceEntitiesFilter) Decode(value interface{}) error {
    if len(model.data) <= 0 {
