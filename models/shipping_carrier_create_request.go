...
// ShippingCarrierCreateRequest A new carrier — the tracking-URL template,
// service level, transit time and pickup cut-off a method inherits.
type ShippingCarrierCreateRequest struct {
    ...
    // jsonb column derives to — this one is always a list. ANDed with the
    // method's own restriction: a method may not be offered into a country its
...
func (model ShippingCarrierCreateRequest) New(data []byte) *ShippingCarrierCreateRequest {
    model.data = data
    return &model
}

func (model *ShippingCarrierCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
