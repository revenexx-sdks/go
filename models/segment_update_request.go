...
// SegmentUpdateRequest Partial update — omitted fields keep their current
// value.
...
func (model SegmentUpdateRequest) New(data []byte) *SegmentUpdateRequest {
    model.data = data
    return &model
}

func (model *SegmentUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
