...
// PaymentMethodUpdateRequest Partial update — omitted fields keep their
// current value.
...
func (model PaymentMethodUpdateRequest) New(data []byte) *PaymentMethodUpdateRequest {
    model.data = data
    return &model
}

func (model *PaymentMethodUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
