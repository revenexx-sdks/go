...
// ShippingMethodCreateRequest A new shipping method — fixed, free or matrix
// pricing.
type ShippingMethodCreateRequest struct {
...
func (model ShippingMethodCreateRequest) New(data []byte) *ShippingMethodCreateRequest {
    model.data = data
    return &model
}

func (model *ShippingMethodCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
