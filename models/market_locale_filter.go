...
// MarketLocaleFilter The exact-column filters this call applied, echoed back.
// Every value is the raw query string, never the column's own type:
...
type MarketLocaleFilter struct {
...
func (model MarketLocaleFilter) New(data []byte) *MarketLocaleFilter {
    model.data = data
    return &model
}

func (model *MarketLocaleFilter) Decode(value interface{}) error {
    if len(model.data) <= 0 {
