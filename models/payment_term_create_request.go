...
// PaymentTermCreateRequest Add one value to the payment terms set. It is
// available to `organizations.payment_terms` immediately.
...
func (model PaymentTermCreateRequest) New(data []byte) *PaymentTermCreateRequest {
    model.data = data
    return &model
}

func (model *PaymentTermCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
