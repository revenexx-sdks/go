...
// AssociationTypesCreateRequest model.
type AssociationTypesCreateRequest struct {
...
func (model AssociationTypesCreateRequest) New(data []byte) *AssociationTypesCreateRequest {
    model.data = data
    return &model
}

func (model *AssociationTypesCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
