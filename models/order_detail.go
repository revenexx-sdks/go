...
// OrderDetail The order aggregate: every column of the order plus its items,
// shipments (with their booked positions), returns and cancellations.
type OrderDetail struct {
    ...
    // What was taken off the order, oldest first — one record per cancel call,
    // whether it emptied the order or removed named quantities. The order-level
    // `cancelled_at` says when it ended; these say what happened.
    ...
    // The positions of the order, in position order. Everything a caller does to
    // this order names one of these by its `id`.
    ...
    // What is coming back or has come back, oldest first, in every state
    // including rejected. A return is registered against the SHIPPED quantities,
    // so this list is empty on an order that never shipped.
    Returns []OrderReturn `json:"returns"`
    // What has gone out, oldest first — each shipment with the position
    // quantities it booked. Empty until something ships; a partial order carries
    // several.
