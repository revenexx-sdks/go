...
// OrderPlaced The order that was created, with its positions. A placement has
// no shipments, returns or cancellations yet — read GET /orders/{id} for
// the aggregate.
type OrderPlaced struct {
    ...
    // The created positions, numbered in steps of the order range position_step
...
func (model OrderPlaced) New(data []byte) *OrderPlaced {
    model.data = data
    return &model
}

func (model *OrderPlaced) Decode(value interface{}) error {
    if len(model.data) <= 0 {
