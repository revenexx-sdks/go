...
// AttributeGroupsFilter The exact-column filters this call was understood to
// carry, verbatim as they arrived. A query parameter that is not a column of
// `attribute_groups` — `?status=`, a typo, a filter another entity has —
...
type AttributeGroupsFilter struct {
...
func (model AttributeGroupsFilter) New(data []byte) *AttributeGroupsFilter {
    model.data = data
    return &model
}

// Use this method to get response in desired type
func (model *AttributeGroupsFilter) Decode(value interface{}) error {
    if len(model.data) <= 0 {
