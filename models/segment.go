...
// Segment A named group of ORGANIZATIONS — by hand, by rule, or both at
// once.
type Segment struct {
...
func (model Segment) New(data []byte) *Segment {
    model.data = data
    return &model
}

func (model *Segment) Decode(value interface{}) error {
    if len(model.data) <= 0 {
