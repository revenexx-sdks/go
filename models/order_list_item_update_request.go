...
// OrderListItemUpdateRequest Partial update — omitted fields keep their
// current value. `quantity` is still checked (> 0) and the article is still
// checked against the catalogue where `reject_unknown_articles` is on.
type OrderListItemUpdateRequest struct {
...
func (model OrderListItemUpdateRequest) New(data []byte) *OrderListItemUpdateRequest {
    model.data = data
    return &model
}

func (model *OrderListItemUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
