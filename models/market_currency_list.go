...
// MarketCurrencyList One page of currencies of a market, the page it sits on,
// and the filters that produced it.
type MarketCurrencyList struct {
    ...
    Filter MarketCurrencyFilter `json:"filter"`
    // The currencies of a market on this page, in `order` — by `position`
    // ascending unless the call asked otherwise.
    Items []MarketCurrency `json:"items"`
...
func (model MarketCurrencyList) New(data []byte) *MarketCurrencyList {
    model.data = data
    return &model
}

func (model *MarketCurrencyList) Decode(value interface{}) error {
    if len(model.data) <= 0 {
