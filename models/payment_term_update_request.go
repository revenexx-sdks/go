...
// PaymentTermUpdateRequest Everything but `code`. Sending a different one is
// a 400 rather than a silent no-op, because records already store it.
...
func (model PaymentTermUpdateRequest) New(data []byte) *PaymentTermUpdateRequest {
    model.data = data
    return &model
}

func (model *PaymentTermUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
