...
// LifecycleStageUpdateRequest Everything but `code`. Sending a different one
// is a 400 rather than a silent no-op, because records already store it.
type LifecycleStageUpdateRequest struct {
...
func (model LifecycleStageUpdateRequest) New(data []byte) *LifecycleStageUpdateRequest {
    model.data = data
    return &model
}

func (model *LifecycleStageUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
