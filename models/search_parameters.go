...
// SearchParameters Typesense search parameters. Only the commonly used ones
// are enumerated — the proxy forwards the whole object, so any parameter
// Typesense accepts may be sent.
...
func (model SearchParameters) New(data []byte) *SearchParameters {
    model.data = data
    return &model
}

// Use this method to get response in desired type
func (model *SearchParameters) Decode(value interface{}) error {
    if len(model.data) <= 0 {
