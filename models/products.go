...
type Products struct {
    ...
    // The display name, maintained by the DATABASE so a grid of twenty thousand
    // rows can sort and filter on a name with no join. It is the first of
    // `attribute_values.common.name`, `…common.label`, the
    // `de`/`en`/`de_DE`/`en_US` locale buckets, `…common.manufacturer_aid`, and
    // finally the SKU — so a value is ALWAYS present, and a label equal to the
    // SKU means the catalog holds no name for this product. A generated column:
    // it cannot be written, and a create or update that names it has it dropped
    // rather than refused. The family-aware answer, which can consult
    // `families.label_attribute` and report where the name came from, is `POST
    // /products/labels`.
