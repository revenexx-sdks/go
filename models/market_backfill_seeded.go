...
// MarketBackfillSeeded Rows this call added that were copied from nowhere,
...
type MarketBackfillSeeded struct {
...
func (model MarketBackfillSeeded) New(data []byte) *MarketBackfillSeeded {
    model.data = data
    return &model
}

func (model *MarketBackfillSeeded) Decode(value interface{}) error {
    if len(model.data) <= 0 {
