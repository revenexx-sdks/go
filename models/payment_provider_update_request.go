...
// PaymentProviderUpdateRequest Partial update — omitted fields keep their
// current value.
type PaymentProviderUpdateRequest struct {
    ...
    // and is worth changing when a tenant runs two accounts with one PSP. Written
    // straight to the database, which refuses an empty one.
...
func (model PaymentProviderUpdateRequest) New(data []byte) *PaymentProviderUpdateRequest {
    model.data = data
    return &model
}

func (model *PaymentProviderUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
