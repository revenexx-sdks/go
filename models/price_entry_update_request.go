...
// PriceEntryUpdateRequest Partial update — omitted fields keep their
// current value.
type PriceEntryUpdateRequest struct {
...
func (model PriceEntryUpdateRequest) New(data []byte) *PriceEntryUpdateRequest {
    model.data = data
    return &model
}

func (model *PriceEntryUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
