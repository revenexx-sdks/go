...
// IoProfileCreateRequest model.
type IoProfileCreateRequest struct {
...
func (model IoProfileCreateRequest) New(data []byte) *IoProfileCreateRequest {
    model.data = data
    return &model
}

func (model *IoProfileCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
