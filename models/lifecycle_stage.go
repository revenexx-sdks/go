...
// LifecycleStage One value of the lifecycle stages set. Where a company
// stands in the sales pipeline — a separate axis from status, and one whose
// steps are a sales team's own.
type LifecycleStage struct {
    // What `organizations.lifecycle_stage` stores, and the only part of this row
    // other data depends on. Immutable once created: renaming it would orphan
    // every record carrying it.
...
func (model LifecycleStage) New(data []byte) *LifecycleStage {
    model.data = data
    return &model
}

func (model *LifecycleStage) Decode(value interface{}) error {
    if len(model.data) <= 0 {
