...
// AttributesCreateRequest model.
type AttributesCreateRequest struct {
...
func (model AttributesCreateRequest) New(data []byte) *AttributesCreateRequest {
    model.data = data
    return &model
}

func (model *AttributesCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
