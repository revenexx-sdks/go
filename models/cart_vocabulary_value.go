...
// CartVocabularyValue model.
type CartVocabularyValue struct {
...
func (model CartVocabularyValue) New(data []byte) *CartVocabularyValue {
    model.data = data
    return &model
}

func (model *CartVocabularyValue) Decode(value interface{}) error {
    if len(model.data) <= 0 {
