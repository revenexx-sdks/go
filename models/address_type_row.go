...
// AddressTypeRow One value of the address types set. What an address is used
// for. Billing and shipping are what a checkout needs; a works entrance or a
// central accounts office is the tenant's own.
...
func (model AddressTypeRow) New(data []byte) *AddressTypeRow {
    model.data = data
    return &model
}

func (model *AddressTypeRow) Decode(value interface{}) error {
    if len(model.data) <= 0 {
