...
// AddressTypeRowCreateRequest Add one value to the address types set. It is
// available to `addresses.type` immediately.
type AddressTypeRowCreateRequest struct {
    // What `addresses.type` will store. Lowercase, starting with a letter;
...
func (model AddressTypeRowCreateRequest) New(data []byte) *AddressTypeRowCreateRequest {
    model.data = data
    return &model
}

func (model *AddressTypeRowCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
