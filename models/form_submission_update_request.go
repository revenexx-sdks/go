...
// FormSubmissionUpdateRequest Partial update — typically the inbox status
// change.
type FormSubmissionUpdateRequest struct {
...
func (model FormSubmissionUpdateRequest) New(data []byte) *FormSubmissionUpdateRequest {
    model.data = data
    return &model
}

func (model *FormSubmissionUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
