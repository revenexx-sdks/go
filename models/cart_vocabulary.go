...
// CartVocabulary model.
type CartVocabulary struct {
    ...
    Values []CartVocabularyValue `json:"values"`

    // Used by Decode() method
    data []byte
}

func (model CartVocabulary) New(data []byte) *CartVocabulary {
    model.data = data
    return &model
}

func (model *CartVocabulary) Decode(value interface{}) error {
    if len(model.data) <= 0 {
