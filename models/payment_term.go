...
// PaymentTerm One value of the payment terms set. When a company has to pay.
// A wholesaler who agrees net 45 with one customer used to need a release of
// this app to say so.
type PaymentTerm struct {
    // What `organizations.payment_terms` stores, and the only part of this row
...
func (model PaymentTerm) New(data []byte) *PaymentTerm {
    model.data = data
    return &model
}

func (model *PaymentTerm) Decode(value interface{}) error {
    if len(model.data) <= 0 {
