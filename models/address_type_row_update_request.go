...
// AddressTypeRowUpdateRequest Everything but `code`. Sending a different one
// is a 400 rather than a silent no-op, because records already store it.
type AddressTypeRowUpdateRequest struct {
...
func (model AddressTypeRowUpdateRequest) New(data []byte) *AddressTypeRowUpdateRequest {
    model.data = data
    return &model
}

func (model *AddressTypeRowUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
