...
// OrderReturnCompleted The completed return plus the restock report.
// Restocking itself is an explicit inventories.restock call by the
// orchestrator — this app books quantity_returned and says what came back,
// it does not write another app's stock.
type OrderReturnCompleted struct {
    ...
    // One entry per returned position that carried restock: true. Empty when
    // nothing was flagged.
    Restock []OrderRestockPosition `json:"restock"`
...
func (model OrderReturnCompleted) New(data []byte) *OrderReturnCompleted {
    model.data = data
    return &model
}

func (model *OrderReturnCompleted) Decode(value interface{}) error {
    if len(model.data) <= 0 {
