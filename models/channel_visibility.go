...
type ChannelVisibility struct {
    ...
    // it.
    Counts ChannelVisibilityCounts `json:"counts"`
    ...
    Items []ChannelVisibilityDecision `json:"items"`
...
func (model ChannelVisibility) New(data []byte) *ChannelVisibility {
    model.data = data
    return &model
}

func (model *ChannelVisibility) Decode(value interface{}) error {
    if len(model.data) <= 0 {
