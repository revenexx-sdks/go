...
// ShippingRateTierUpdateRequest Partial update — omitted fields keep their
// current value.
type ShippingRateTierUpdateRequest struct {
...
func (model ShippingRateTierUpdateRequest) New(data []byte) *ShippingRateTierUpdateRequest {
    model.data = data
    return &model
}

func (model *ShippingRateTierUpdateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
