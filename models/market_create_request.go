...
// MarketCreateRequest A market needs a 'code' and a 'name' — currency
// defaults to EUR, status to active. To get a market that can actually trade,
// clone an existing one instead: POST /markets/{id}/clone.
...
func (model MarketCreateRequest) New(data []byte) *MarketCreateRequest {
    model.data = data
    return &model
}

func (model *MarketCreateRequest) Decode(value interface{}) error {
    if len(model.data) <= 0 {
