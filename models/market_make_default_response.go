...
// MarketMakeDefaultResponse The market as it now stands, plus what had to
// move out of its way.
type MarketMakeDefaultResponse struct {
...
func (model MarketMakeDefaultResponse) New(data []byte) *MarketMakeDefaultResponse {
    model.data = data
    return &model
}

func (model *MarketMakeDefaultResponse) Decode(value interface{}) error {
    if len(model.data) <= 0 {
